...
- [ ] **0.3** 布局引擎重构 — 声明式（App.tsx）
  - [ ] 0.3.1 提取布局模式逻辑到 layoutStore
  - [ ] 0.3.2 layoutStore 添加 `mode` 状态、`setMode` action、状态快照（conversationSnapshot / editorSnapshot）
  - [ ] 0.3.3 实现 `LayoutEngine` 通用渲染器（从 `layoutRegistry` 数据驱动渲染，零 if-else）
  - [ ] 0.3.4 初始状态 = `split`（兼容现有行为）
  - [ ] 0.3.5 模式切换时保存/恢复各模式的状态快照
  - [ ] 0.3.6 模式切换的 300ms 滑动动画
  - [ ] 0.3.7 5 种切换触点的统一注册（triggers config array → 批量绑定，非 5 个独立 handler）
...
- [ ] **0.5.15** Registry 单元测试（UT1.1，12 用例）
  - [ ] 0.5.15.1 注册 + 查询 + 覆盖
...
- [ ] **0.5.16** DSL Compiler 单元测试（UT1.2-1.6，76 用例）
...
- [ ] **0.5.17** workflowStore 单元测试（UT1.8，10 用例）
...
- [x] **1.5** 右栏 — Tab 并排三面板（基于真实数据） ✅
  ...
  - [x] 1.5.6 Tab 并排三标签切换（工作日志 / 产出物 / 预览） ✅
...
- [x] **1.7** 布局模式集成 ✅
  ...
  - [x] 1.7.4 底部快捷切换按钮 ✅ (LayoutModeBar DSL 查表, LMB-1~7 测试)
  - [x] 1.7.5 键盘快捷键（⌘+1/2/3） ✅ (LayoutShortcuts KEY_TO_MODE 查表, LS-1~7 测试)
  - [ ] 1.7.6 右键菜单布局模式子菜单
  - [ ] 1.7.7 文件引用上下文菜单（点击文件链接显示操作选项）
  - [ ] 1.7.8 模式切换 300ms 滑动动画

- [ ] **1.8** split 模式改进（当前布局的进化版）
  - [ ] 1.8.1 聊天面板采用新设计的对话气泡
  - [ ] 1.8.2 聊天面板宽度自适应
  - [ ] 1.8.3 聊天面板可折叠为浮动按钮

- [ ] **1.9** 组件测试
  - [ ] 1.9.1 用户消息气泡渲染测试（品牌蓝背景 + 右对齐 + 圆角）
  - [ ] 1.9.2 AI 消息气泡渲染测试（中性色背景 + 左对齐 + Agent 头像）
  - [ ] 1.9.3 消息列表滚动测试（20 条以上自动到底部）
  - [ ] 1.9.4 输入框自适应高度测试（多行递增至 8 行上限）
  - [ ] 1.9.5 空对话占位测试（无消息时显示"开始新对话"）
  - [x] 1.9.6 时间分组渲染测试 ✅（TG-1~11, TD-1~5）
  - [x] 1.9.7 三栏布局单元测试 + 宽度拖拽测试 ✅（LR-8~14, layoutStore LR-1~7）
  - [x] 1.9.8 左栏对话列表渲染 + 选择测试 ✅（CLP-1~13）
  - [x] 1.9.9 右栏面板测试 ✅（WLP-1~5, AP-1~5, PP-1~4, CDP-1~7, RL-1~7, RA-1~6）
  - [ ] 1.9.10 面板拖拽调整高度 E2E 测试（IT3.12）
  - [ ] 1.9.11 三栏宽度拖拽 E2E 测试（IT3.13）
  - [ ] 1.9.12 布局模式切换 E2E 测试（E2E5.8）
  - [ ] 1.9.13 Fidelity 验证：FT4.1（初始对话界面）+ FT4.2（用户气泡）+ FT4.3（AI 气泡）
...
- [ ] **2.2** Agent 工作台组件
  ...
  - [ ] 2.2.2 实现 `AgentWorkstation.tsx` 单个 Agent 工位（stateStyle 查表驱动，禁用三元链）
  - [ ] 2.2.3 实现 `AgentAvatar.tsx` SVG 角色形象
  - [ ] 2.2.4 实现 `AgentStatusIndicator.tsx` 状态指示器（statusStyleMap 查表：绿=工作中/灰=空闲/红=错误）
  - [ ] 2.2.5 实现工作台场景背景（WorkspaceScene.tsx）
...
- [ ] **2.8** 工作台集成
  - [ ] 2.8.1 conversation 模式下 AgentWorkspace 可折叠
  - [ ] 2.8.2 split 模式下 AgentWorkspace 在聊天面板顶部
  - [ ] 2.8.3 工作台大小自适应（4/5/6 Agent 布局）
  - [ ] 2.8.4 空状态设计（无活跃 Agent 时的占位动画）
...
- [ ] **2.10** ApprovalCard 组件测试（CT2.1，12 用例）
  - [ ] 2.10.1 低/中/高 3 种风险渲染（左边框色、徽章色、提示文字）
  - [ ] 2.10.2 文件列表渲染（files=3 个 vs 空列表）
  - [ ] 2.10.3 批准交互（按钮变绿 → 勾号动画 → 1.2s → resolve）
  - [ ] 2.10.4 批准后按钮禁用（Approve + Reject 均 disabled）
  - [ ] 2.10.5 拒绝交互（卡片变红 → 显示 consequence 选项）
  - [ ] 2.10.6 跳过后果（Reject → skip → 1.0s → resolve）
  - [ ] 2.10.7 回退后果（Reject → rollback → 1.0s → resolve）
  - [ ] 2.10.8 终止后果（Reject → abort → 1.0s → resolve）
  - [ ] 2.10.9 批准后拒绝不可用
  - [ ] 2.10.10 Fidelity：FT4.6（低风险卡片）+ FT4.7（高风险卡片）+ FT4.8（批准后）+ FT4.9（拒绝后）

- [ ] **2.11** FilePreview 组件测试（CT2.2，8 用例）
...
- [ ] **2.12** InteractionCard 组件测试（CT2.3，10 用例）
  - [ ] 2.11.1 单选 3 个 option 渲染
  - [ ] 2.11.2 单选选中高亮 + 其他半透明 + 0.8s resolve
  - [ ] 2.11.3 单选切换（点 A → 点 B，A 恢复）
  - [ ] 2.11.4 选项标签渲染（tag/tagColor 正确配色）
  - [ ] 2.11.5 多选 checkbox + 确认按钮渲染
  - [ ] 2.11.6 多选勾选 + 计数更新
  - [ ] 2.11.7 多选取消勾选
  - [ ] 2.11.8 多选确认后禁用 + 变绿 + 1.0s resolve
  - [ ] 2.11.9 多选 0 项确认按钮禁用
  - [ ] 2.11.10 Fidelity：FT4.10（单选渲染）+ FT4.11（单选选中）+ FT4.12（多选确认）

- [ ] **2.12** AgentWorkstation 组件测试（CT2.3，8 用例）
  - [ ] 2.12.1 idle/working/error/celebrating 4 种状态渲染
  - [ ] 2.12.2 celebrating 500ms 后自动 idle
  - [ ] 2.12.3 紧凑模式（32px height + 圆点）
  - [ ] 2.12.4 展开模式（完整工位 + 头像 + 进度）
  - [ ] 2.12.5 进度条渲染（progress=65 → 填充 65%）
  - [ ] 2.12.6 任务描述文本
  - [ ] 2.12.7 Fidelity：FT4.4（紧凑栏）+ FT4.5（展开工作台）

- [ ] **2.13** 集成测试（IT3.5-3.18，14 场景）
