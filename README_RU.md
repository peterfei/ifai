...
### 🌟 v0.5.2 Новые возможности: Новый GUI режим диалога + Система управления потоками + Агентное сотрудничество

**I. Новый GUI режим диалога 💬 (главная функция v0.5.2)**
- **Трёхпанельная компоновка** — Список диалогов (слева) + AI чат (центр) + Панель деталей (справа: журнал / артефакты / предпросмотр)
- **Многопоточный параллельный чат** — Несколько диалогов одновременно, потоковая изоляция
- **Быстрые операции с потоками** — Ctrl+T создать / Ctrl+Tab переключить / F2 переименовать / правый клик архивировать/удалить
- **Индикатор непрочитанных** — Автоматическая отметка фоновых сообщений, сброс при возврате
- **Регулируемая компоновка** — Левая/правая панели перетаскиваются (150-600px), сворачиваются одним кликом
- **Восстановление после перезапуска** — Автосохранение в IndexedDB, полное восстановление

**II. Декларативная DSL архитектура**
- **Архитектура двойного реестра** — `layoutRegistry` + `componentRegistry`, ноль if-else ветвлений
- **24 декларативные DSL таблицы** — Все UI поведение на основе таблиц
- **Карточный конвейер** — 10 типов карточек сообщений с регистрацией во время выполнения
- **Система цветовых токенов** — `PALETTE_DSL` 9-уровневая прозрачность текста

**III. Система управления потоками**
- **Двухочередной MessageQueue** — Потоковая конкуренция, последовательная + параллельная
- **5-мерное обнаружение активности** — stream / per-thread / agent / tool / workflow
- **Кросс-потоковая маршрутизация** — Полная цепочка событий workflow маршрутизируется между потоками
- **StreamingPulseBanner** — per-thread streamSummary с межпотоковым сохранением

**IV. Агентное сотрудничество**
- **`call_agent_parallel`** — Параллельный вызов агентов
- **`share_knowledge` + `aggregate_results`** — Обмен и агрегация результатов
- **Автосотрудничество** — Агенты авто-вызывают специализированных агентов (глубина до 5)

**V. Улучшения GUI**
- **Skills Hub**: 8 категорий + полноэкранный модал
- **Система анимации агентов**: 7 агентов × 6 анимаций × 4 уровня деградации
- **Авторизация файлов**: PermissionStore + ApprovalCard
- **Встроенный предпросмотр**: iframe двухрежимный
...
| :--- | :--- | :--- |
| **v0.5.2** | **Новый GUI режим диалога + Управление потоками + Агентное сотрудничество** | **GUI декларативная DSL архитектура (24 DSL таблицы + двойной реестр + карточный конвейер), Управление потоками (MessageQueue + 5-мерное обнаружение + кросс-потоковая маршрутизация + индикатор непрочитанных + StreamingPulseBanner), Агентное сотрудничество (call_agent_parallel + share_knowledge + aggregate_results), Workflow движок (YAML + DAG), Skills Hub, Система анимации агентов, Авторизация файлов, Встроенный предпросмотр** |
