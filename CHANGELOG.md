...
## [0.2.1] - 2025-12-24 - 架构解耦，双版本发布支持

### 新增
- 引入 **Feature Flag** 系统，支持 `community` 和 `commercial` 两种编译特性
- 核心抽象层：在 `src-tauri` 中新增 `core_traits.rs` 定义标准接口
- 前端 Mock：新增 `src/core/mock-core.ts` 支持社区版独立运行
- CI/CD：发布工作流现在支持自动化构建商业版

### 修复
- 修复了商业版与社区版混合编译导致的类型冲突问题
- 修复了社区版下 RAG 初始化导致的 Promise 未捕获异常
- 修正了 `npm run dev` 递归启动死循环的问题
- 修复了提示词管理在商业版下的锁定显示逻辑错误
