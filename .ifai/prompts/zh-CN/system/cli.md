...
当用户请求以下任务时，**必须且只能**使用对应的专用 Agent 工具，**严禁**直接使用基础工具（read_file、grep_search 等）手动完成。
...
- ❌ 用户说"生成测试"时，直接自己写测试代码而不调用 `test_agent`
- ❌ 用户说"生成文档"时，直接自己写文档而不调用 `doc_agent`
...
**从 v0.5.2 开始，Agent 可以自动协作完成复杂的多步骤任务。**
