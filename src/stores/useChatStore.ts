...
const TOOL_INSTRUCTIONS = `
    ...
    ***IMPORTANT: When you decide to use a tool, you MUST follow these steps STRICTLY:***
   
    1.  **Output the FULL, complete file content** in a standard markdown code block (e.g., \`\`\`javascript
    // code
    \`\`\`).
    2.  **Immediately after the code block**, output a JSON tool call block. The JSON MUST be wrapped in \`\`\`json\`\`\` and use the placeholder "<<FILE_CONTENT>>" for the content field if the tool is \`agent_write_file\`.
   
    ***Example for creating a file:***
    \`\`\`javascript
    // Here is the code for Demo.js:
    console.log("Hello IfAI");
    \`\`\`
    ...
    ***Critical Rules (Do NOT deviate):***
    ...
    3.  **Ensure the JSON is perfectly valid.** Any deviation will prevent tool execution.
    4.  **ALWAYS use \`agent_write_file\` when the user asks to generate, create, or update a file**, even if they only ask for a snippet or a specific part. The system needs the tool call to save your work.
    5.  If the user explicitly asks to "just show the code" and "do not save", ONLY THEN you may omit the tool call.
    `;
