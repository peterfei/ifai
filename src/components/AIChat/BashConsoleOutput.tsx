...
export const BashConsoleOutput: React.FC<BashConsoleOutputProps> = ({
  ...
  return (
    <div className={`bash-console-output ${className}`}>
      {/* 控制台头部 */}
      <div className="flex items-center justify-between px-4 py-2 theme-panel-muted border-b theme-border rounded-t-xl">
        <div className="flex items-center gap-2">
          <Terminal className="w-4 h-4 theme-text-subtle" />
          <span className="text-xs font-medium theme-text-muted">Console Output</span>
          {exitCode !== undefined && (
      ...
      >
        ...
        <div className="space-y-0.5">
          {logLines.map((line, index) => (
            <div
              key={index}
              className="group flex items-start theme-soft-hover -mx-1 px-1 py-0.5 rounded transition-colors"
            >
              {/* 行号 */}
              ...
              {/* 内容 */}
              <span className="flex-1 theme-text-muted leading-5 break-words font-mono whitespace-pre-wrap">
                {line.content}
              </span>
            </div>
          ))}
      ...
      <style dangerouslySetInnerHTML={{ __html: `
        ...
        .bash-console-output ::-webkit-scrollbar-track {
          background: ${dark ? 'rgba(26, 26, 26, 0.5)' : 'rgba(226, 232, 240, 0.8)'};
          border-radius: 4px;
        }

        .bash-console-output ::-webkit-scrollbar-thumb {
          background: ${dark ? 'rgba(102, 102, 102, 0.5)' : 'rgba(148, 163, 184, 0.8)'};
          border-radius: 4px;
        ...
        .bash-console-output ::-webkit-scrollbar-thumb:hover {
          background: ${dark ? 'rgba(120, 120, 120, 0.6)' : 'rgba(100, 116, 139, 0.9)'};
          border: 2px solid transparent;
          background-clip: content-box;
        }
