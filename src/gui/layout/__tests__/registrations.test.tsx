...
describe('Layout registrations', () => {
  ...
  it('所有面板组件均已注册', () => {
    const ids = ['conversation-list', 'conversation', 'conversation-detail', 'editor', 'agent-workspace', 'conversation-task'];
    for (const id of ids) {
  ...
  it('conversation-list 和 conversation-detail 组件可渲染', () => {
    const ids = ['conversation-list', 'conversation-detail'];
    for (const id of ids) {
      const Comp = componentRegistry.get(id);
      expect(Comp, `Component "${id}" should be registered`).toBeDefined();
      if (Comp) {
