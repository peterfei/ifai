...
impl SessionPersistence {
    ...
    pub fn load_session(&self, id: &str) -> Result<DebugSession, String> {
        ...
        let json = fs::read_to_string(file_path).map_err(|e| format!("读取文件失败: {}", e))?;

        let session = serde_json::from_str(&json).map_err(|e| format!("反序列化失败: {}", e))?;
