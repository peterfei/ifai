...
**目标**：全面理解项目架构、技术栈和关键模块

**工具使用策略**（充分利用并行能力）：

⚠️ **关键**：在同一次响应中发起多个 tool_call，它们会并行执行！

第 1 轮（快速定位）：
- 同时读取 3-5 个关键文件：配置文件、入口文件、核心模块
- 使用 `agent_read_file`，一次性发起所有调用
...
- `agent_search(pattern, path)` — 搜索代码（查找函数/类的定义和使用）

**探索原则**：
1. **优先并行**：一次响应发起 3-10 个工具调用，充分利用并行能力
2. **分层次**：先配置→再核心→再细节
3. **看重点**：关注业务逻辑、数据流、架构模式
4. **控制大小**：单个文件 < 5000 行，总计 < 20 个文件
