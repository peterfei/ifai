...
impl AgentPromptLoader {
    ...
    fn get_config_for_agent(
        ...
        match agent_type {
            ...
            AgentType::GitCommit => AgentPromptConfig {
...
- **总共最多 2 轮响应**（2 次 assistant turn）
- **第 1 轮**：最多并行读取 **3 个**报错直接相关的文件（不要读无关文件！）
- **第 2 轮**：不调用任何工具，直接输出诊断分析
- **禁止**读取与错误无关的文件（如 README、Cargo.toml、mod.rs 索引文件）

## 工具使用策略

第 1 轮（tool_call）：
- 只用 `agent_read_file` 读取出错文件
- 根据错误信息中的文件路径和行号，精准选择 **最多 3 个**文件
- 如果错误信息未指定文件，先用 `grep_search` 定位，再读取
...
- `agent_read_file(rel_path)` — 读取单个文件
- `grep_search(pattern, path)` — 搜索代码内容

## 调试分析流程

1. **定位错误**：根据错误信息定位出错文件和行号
2. **根因分析**：读取相关代码，分析错误根因
...
fn fallback_git_commit_prompt(ctx: &AgentContext) -> String {
    format!(
        r#"你是一个专业的 Git 提交助手，负责安全地提交代码变更。

## 关键：只能通过 git_commit 工具提交
...
2. 调用 agent_read_file 读取变更文件内容
3. 调用 secret_scanner 扫描所有变更内容，确保无敏感信息
4. 调用 git_snapshot 创建快照（用于回滚）
5. 调用 git_commit 执行提交（自动 git add -A + 追加 Co-authored-by）
