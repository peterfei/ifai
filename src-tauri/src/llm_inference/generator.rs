...
use crate::llm_inference::{model::Model, InferenceError};
    ...
    pub fn generate(&self, prompt: &str, model: &Model) -> Result<String, InferenceError> {
        ...
        for (i, token) in (0_i32..).zip(tokens_list.into_iter()) {
            ...
            batch.add(token, i, &[0], is_last).map_err(|e| {
                InferenceError::InferenceFailed(format!("添加 token 到批处理失败: {}", e))
            })?;
        }
        ...
        println!(
            "[TextGenerator] Generated {} tokens, {} chars",
            n_decode,
            result.len()
        );
        Ok(result)
...
pub fn generate_completion(prompt: &str, max_tokens: usize) -> Result<String, InferenceError> {
    use crate::llm_inference::model::{ensure_model_loaded, get_or_init_model};
    ...
    let model_guard = model_ref
        .lock()
        .map_err(|_| InferenceError::InferenceFailed("获取模型锁失败".to_string()))?;

    let model = model_guard.as_ref().ok_or(InferenceError::ModelNotLoaded)?;

    // 创建生成器并生成
    let generator = TextGenerator::new().with_max_tokens(max_tokens);
...
mod tests {
    ...
    fn test_generator_creation() {
        let generator = TextGenerator::new().with_max_tokens(100).with_seed(42);
