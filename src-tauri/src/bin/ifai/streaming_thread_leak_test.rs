...
mod tests {
    ...
    fn test_streaming_output_leaks_to_thread2() {
        ...
        // 🔥 关键：记录用户输入时的线程 ID
        let request_thread_id = app.thread_store.active_thread()
            .map(|t| t.id)
            .unwrap_or_else(|| app.thread_store.primary_id());

        // 存储到 thread_messages
        app.thread_messages.push(request_thread_id, crate::thread::Message::user(user_input.to_string()));
        ...
        app.thread_messages.push(request_thread_id, crate::thread::Message::user(streaming_line_1.to_string()));
        ...
        app.thread_messages.push(request_thread_id, crate::thread::Message::user(streaming_line_2.to_string()));
    ...
    fn test_streaming_output_leaks_bug_scenario() {
        ...
        let request_thread_id = app.thread_store.active_thread().map(|t| t.id).unwrap();

        app.thread_messages.push(request_thread_id, crate::thread::Message::user("Question about Rust".to_string()));
        ...
        let wrong_thread_id = app.thread_store.active_thread().map(|t| t.id).unwrap();

        // 模拟错误：使用 wrong_thread_id（当前活动线程 ID）
        app.thread_messages.push(wrong_thread_id, crate::thread::Message::user(" a systems".to_string()));
    ...
    fn test_correct_streaming_with_thread_switch() {
        ...
        app.thread_messages.push(request_thread_id, crate::thread::Message::user("Tell me about Go".to_string()));

        // === 流式输出第一行 ===
        app.thread_messages.push(request_thread_id, crate::thread::Message::user("Go is".to_string()));
        ...
        app.thread_messages.push(request_thread_id, crate::thread::Message::user(" a language".to_string()));

        // === 用户切换到 Thread-3 ===
        app.switch_thread(thread3_id);

        // === 流式输出第三行 ===
        app.thread_messages.push(request_thread_id, crate::thread::Message::user(" by Google".to_string()));
    ...
    fn test_real_world_streaming_scenario() {
        ...
        // === 步骤 1：用户在主线程输入 ===
        app.switch_thread(primary_id);
        app.thread_messages.push(primary_id, crate::thread::Message::user("Explain Rust ownership".to_string()));
        ...
        // 第 1 行（用户还在主线程）
        app.thread_messages.push(request_thread_id, crate::thread::Message::user("Rust's ownership system".to_string()));

        // 用户切换到 Thread-2
        app.switch_thread(thread2_id);

        // 第 2 行（用户在 Thread-2，但消息应该去主线程）
        app.thread_messages.push(request_thread_id, crate::thread::Message::user(" is a way to manage".to_string()));

        // 用户再切换回主线程
        app.switch_thread(primary_id);

        // 第 3 行（用户在主线程）
        app.thread_messages.push(request_thread_id, crate::thread::Message::user(" memory without GC".to_string()));
    ...
    fn test_code_verification_request_thread_id_fix() {
        ...
        app.thread_messages.push(request_thread_id, crate::thread::Message::user(streaming_content.to_string()));
