...
#[tokio::test]
#[serial_test::serial]
async fn test_liu_shizhong_duan() {
    // 验证用户 Ctrl+C 中断流式响应
    let mut env = TestEnv::new().await.unwrap();
    env.set_stdin("\u{3}");
...
#[tokio::test]
#[serial_test::serial]
async fn test_liu_shixiang_ying_gong_judiao_yong() {
    // 验证流式响应中包含工具调用事件
    let mut env = TestEnv::new().await.unwrap();
...
#[tokio::test]
#[serial_test::serial]
async fn test_wen_benliu_shichuan_shu() {
    // 验证大文本的流式传输不丢失数据
    let mut env = TestEnv::new().await.unwrap();
...
#[tokio::test]
#[serial_test::serial]
async fn test_liu_shixiang_ying_dai_makuai() {
