...
pub mod impls {
    ...
    use crate::core_traits::rag::{RagResult, RagService};
    use ifainew_core;
    ...
    impl RagService for CommercialRagService {
        async fn index_project(&self, root: &str) -> Result<(), String> {
            ...
            ifainew_core::rag::init_rag_index(self.app.clone(), state, root.to_string())
                .await
                .map(|_| ())
        }
        ...
        async fn retrieve_context(&self, query: &str, root: &str) -> Result<RagResult, String> {
            ...
            let core_res =
                ifainew_core::rag::build_context(state, query.to_string(), root.to_string())
                    .await?;
