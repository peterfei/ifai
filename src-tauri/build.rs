...
fn main() {
    ...
    println!("cargo:rerun-if-changed=schemas/events.yaml");

    // 生成 CollabEvent 枚举代码
    if let Err(e) = generate_events() {
        println!("cargo:warning=事件代码生成失败: {}", e);
    }
...
/// 生成 CollabEvent 枚举代码
fn generate_events() -> Result<(), Box<dyn std::error::Error>> {
    let schema_path = Path::new("schemas/events.yaml");
    if !schema_path.exists() {
        println!("cargo:warning=⚠️  事件 schema 不存在: {:?}", schema_path);
        return Ok(());
    }
    ...
    let events = schema
        ...
        .ok_or("Missing events sequence")?;

    let out_dir = env::var("OUT_DIR")?;
    let output_path = Path::new(&out_dir).join("events_gen.rs");

    let mut code = String::new();
    code.push_str("// Generated by build.rs from schemas/events.yaml\n");
    code.push_str("// DO NOT EDIT MANUALLY\n\n");

    // 生成 CollabEvent 枚举
    code.push_str("use serde::{Deserialize, Serialize};\n\n");
    code.push_str("/// Agent 协作事件\n");
    code.push_str("/// 自动生成自 schemas/events.yaml\n");
    code.push_str("#[derive(Debug, Clone, Serialize, Deserialize)]\n");
    code.push_str("#[serde(tag = \"type\", content = \"data\")]\n");
    code.push_str("pub enum CollabEvent {\n");

    for event in events {
        let name = event.get("name").and_then(|v| v.as_str()).unwrap();
        let tauri_event = event.get("tauri_event").and_then(|v| v.as_str()).unwrap();
        let desc = event
            .get("description")
            .and_then(|v| v.as_str())
            .unwrap_or("");
        ...
        if let Some(fields) = event.get("fields").and_then(|v| v.as_mapping()) {
            if fields.is_empty() {
                code.push_str(&format!("    {},\n", name));
            } else {
                code.push_str(&format!("    {} {{\n", name));
                for (key, field_info) in fields {
                    let field_name = key.as_str().unwrap();
                    let field_type = field_info
                        .get("type")
                        .and_then(|v| v.as_str())
                        .unwrap_or("String");
                    let field_desc = field_info
                        .get("desc")
                        .and_then(|v| v.as_str())
                        .unwrap_or("");
                    code.push_str(&format!(
    ...
    // 生成 emit_collab_event 宏
    code.push_str("/// 发射 CollabEvent 到 Tauri 前端\n");
    code.push_str("#[macro_export]\n");
    code.push_str("macro_rules! emit_collab_event {\n");
    code.push_str("    ($app:expr, $event:expr) => {{\n");
    code.push_str("        let json = serde_json::to_value($event).expect(\"序列化 CollabEvent 失败\");\n");
    code.push_str("        let event_type = json[\"type\"].as_str().unwrap_or(\"unknown\").to_string();\n");
    code.push_str("        let _ = $app.emit(&event_type, json[\"data\"].clone());\n");
    code.push_str("    }};\n");
    code.push_str("}\n\n");

    // 生成测试工厂函数
    code.push_str("#[cfg(test)]\n");
    code.push_str("pub mod test_factories {\n");
    code.push_str("    use super::CollabEvent;\n\n");

    for event in events {
        let name = event.get("name").and_then(|v| v.as_str()).unwrap();
        let factory_name = format!("make_{}", to_snake_case(name));
        ...
        if let Some(fields) = event.get("fields").and_then(|v| v.as_mapping()) {
            if !fields.is_empty() {
                ...
                code.push_str(&format!("    pub fn {}(", factory_name));
                code.push_str(&params.join(", "));
                code.push_str(") -> CollabEvent {\n");
                code.push_str(&format!("        CollabEvent::{} {{\n", name));
