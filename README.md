...
| 版本 | 日期 | 核心主题 | 关键特性 |
| :--- | :--- | :--- | :--- |
| **v0.2.6** | 2026-01-07 | **Agent 进化** | Shell 能力、任务树、OpenSpec、Markdown 预览、Token 可视化 |
| **v0.2.5** | 2026-01-03 | **混合智能** | 本地 LLM 支持 (Qwen2.5)、智能路由、离线代码补全 |
| **v0.2.4** | 2025-12-30 | **体验极致化** | Windows 渲染深度优化、零闪屏生成、稳定性飞跃 |
| **v0.2.3** | 2025-12-26 | **工业级基座** | 任务监控面板、项目级 `IFAI.md` 配置、虚拟文件树 |
| **v0.2.0** | 2025-12-20 | **性能革命** | 120 FPS 高刷支持、GPU 加速、Claude-style 流式交互 |
...
### 🐚 Agent Shell 级掌控
- **系统级交互**：Agent 现在可以执行 `npm`, `git`, `cargo` 等物理命令，自主完成依赖安装与环境配置。
- **环境自愈**：具备“路径感知”能力，自动识别并跳出 `src-tauri` 源码陷阱，根据错误反馈自动修正工作目录。

### 🌳 结构化任务拆解 (Task Tree)
- **架构师思维**：支持将模糊的自然语言需求拆解为层级分明的任务树，并在 UI 中实时追踪执行进度。
- **持久化同步**：任务状态实时同步至 `.ifai/tasks/`，支持跨会话断点续传。

### 📑 OpenSpec 深度融合
- **规范驱动开发**：原生集成 [OpenSpec](https://openspec.dev) 标准，让 AI 遵循工业级开发规范生成代码。
- **自定义 API**：全面兼容 OpenAI 格式，轻松接入 DeepSeek、Kimi 等主流或私有大模型。

### ⚡ 极致性能与体验
- **Snippet 虚拟滚动**：代码片段管理器支持万级数据秒级加载，保持 120 FPS 丝滑滚动。
- **本地模型增强**：实现 Local LLM 自动续写功能，彻底解决长文本截断痛点。
...
#### 4. **自主 Agent 进化** 🧬
- **Shell 能力**：安全可控的系统级命令执行 (npm/git/cargo)
- **环境感知**：自动识别运行环境并校准执行路径
- **自我纠错**：基于 stderr 分析的自主错误修复机制
- **任务拆解**：复杂需求自动转化为结构化任务树

#### 5. **工业级测试保障** 🧪
- **全链路 E2E**：覆盖 AI 核心流、Agent 工具链与复杂 UI 交互的自动化测试
- **专项实验室**：建立“Agent 循环预防”和“状态隔离”专项回归测试集
- **跨平台 CI**：代码级联构建流水线，确保双平台产物 100% 稳定可靠
...
- **性能压测验证**：万级 Snippet 滚动保持 **120 FPS**，批量处理耗时减少 **68%**

#### 前端优化
- **虚拟滚动**：支持 **10,000+** 列表项流畅滚动，UI 响应延迟 **< 15ms**
- **增量更新**：仅更新变更部分，减少 80% 计算
- **流式负载均衡**：CPU 占用降低 **30%**，即使在高频生成时依然丝滑
