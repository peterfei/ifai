...
test.describe('无项目路径测试', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    await page.evaluate(() => {
      ...
      // 🔥 不设置项目路径（让它为空或 undefined）
      const fileStore = (window as any).__fileStore;
      if (fileStore) {
        const currentRoot = fileStore.getState().rootPath;
        console.log('[TEST] Current project root (before clear):', currentRoot);
        // 不设置或清空项目路径
      }
    });
  });

  test('无项目路径时测试流式响应', async ({ page }) => {
    console.log('[测试] 无项目路径测试');
    ...
    for (let i = 0; i < 60; i++) {
      ...
      if (i % 5 === 0 || check.contentLength > 0) {
        console.log(`[测试] T+${check.elapsedSeconds}s:`, {
          ...
          content: check.content.substring(0, 50),
    ...
    console.log('  - 内容预览:', finalResult.content.substring(0, 200));
    console.log('  - Segments:', finalResult.segments);
