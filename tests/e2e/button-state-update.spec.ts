...
test.describe('Button State Update - Immediate Feedback', () => {
  ...
  test('点击批准按钮后状态应该立即更新为 approved', async ({ page }) => {
  ...
  test('验证批准按钮在点击后立即禁用或改变外观', async ({ page }) => {
    // 测试按钮的视觉反馈
    await page.evaluate(() => {
      const chatStore = (window as any).__chatStore?.getState();
      chatStore.addMessage({
  ...
  test('模拟长时间运行的命令，状态应该持续更新', async ({ page }) => {
