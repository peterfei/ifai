...
test.describe('流式工具参数显示测试', () => {
    ...
    test('应该显示流式参数查看器而非JSON', async ({ page }) => {
        // 发送一个会触发工具调用的消息
        await page.fill('[data-test-id="chat-input"]', '创建一个测试文件 test.txt，内容为 hello world');
    ...
    test('完成后应显示checkbox选中状态', async ({ page }) => {
        // 发送消息
        await page.fill('[data-test-id="chat-input"]', '创建文件 hello.txt');
        await page.click('[data-test-id="send-button"]');
    ...
    test('不应该显示"正在解析工具参数"提示', async ({ page }) => {
        // 发送消息
        await page.fill('[data-test-id="chat-input"]', '读取文件 package.json');
        await page.click('[data-test-id="send-button"]');
    ...
    test('应该正确显示不同类型的参数图标', async ({ page }) => {
        // 发送涉及多种参数类型的消息
        await page.fill('[data-test-id="chat-input"]', '在 /tmp 目录创建文件 test.log');
        await page.click('[data-test-id="send-button"]');
    ...
    test('流式生成时不应该显示上一个工具的结果', async ({ page }) => {
        // 第一个工具调用
        await page.fill('[data-test-id="chat-input"]', '创建文件 a.txt');
        await page.click('[data-test-id="send-button"]');
        ...
        // 第二个工具调用（在同一个消息中）
        await page.fill('[data-test-id="chat-input"]', '创建文件 b.txt');
        await page.click('[data-test-id="send-button"]');
    ...
    test('参数值应被正确截断而不是显示完整JSON', async ({ page }) => {
        ...
        await page.fill('[data-test-id="chat-input"]', `创建文件，内容为：${longContent}`);
        await page.click('[data-test-id="send-button"]');
...
test.describe('流式参数显示UI验证', () => {
    test('应该有正确的样式类', async ({ page }) => {
        ...
        await page.fill('[data-test-id="chat-input"]', '创建测试文件');
        await page.click('[data-test-id="send-button"]');
