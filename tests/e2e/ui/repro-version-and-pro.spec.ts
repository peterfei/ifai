...
test.describe('Reproduction: Version Display and Pro Badge', () => {
  ...
  test('should display dynamic system version and correct Pro badge', async ({ page }) => {
    // 1. 验证版本号是否显示 (当前硬编码为 V0.2.6 PRO,这是需要修复的bug)
    // 使用文本选择器定位版本显示元素
    const versionElement = page.locator('text=V0.2.6').or(page.locator('text=v0.2.6'));
    ...
    // 验证版本包含 0.2.6 (注意:当前是硬编码,这是bug)
    expect(versionText.toUpperCase()).toContain('0.2.6');

    // 2. 验证 PRO 标识是否显示 (当前硬编码显示,这是bug)
    // 使用更精确的选择器,避免匹配到 "mock-project" 等包含 "PRO" 的文本
    const proBadge = page.locator('span:has-text("V0.2.6 PRO")').or(page.locator('text=V0.2.6 PRO'));
    ...
    if (switchSuccessful) {
      ...
      const proBadgeAfter = page.locator('span:has-text("V0.2.6 PRO")').or(page.locator('text=V0.2.6 PRO'));
