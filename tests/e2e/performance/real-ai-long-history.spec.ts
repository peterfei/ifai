...
test.describe('🔬 长历史真实 AI 性能测试', () => {
  ...
  /**
   * 🎯 核心测试：149 条历史 + 真实 AI 流式响应
   *
   * 测试场景：
   * 1. 生成 149 条历史消息（低于自动压缩阈值 150）
   * 2. 发送真实 AI 请求
   ...
   * 🔥 FIX: 复用对比测试的代码逻辑
   */
  test('🔬 [真实 AI] 149 条历史 + 流式响应卡顿检测', async ({ page }) => {
    // 检查 API Key
    ...
    console.log(`[E2E] 📊 历史消息验证: 期望 149 条，实际 ${messageCount} 条`);
      ...
      try {
        ...
        await page.waitForFunction(() => {
          ...
          const partialState = await page.evaluate(() => {
            ...
            const lastMessage = messages[messages.length - 1];
            return {
              hasContent: !!lastMessage?.content && lastMessage.content.length > 0,
              contentLength: lastMessage?.content?.length || 0,
              isStreaming: lastMessage?.isStreaming
            };
          });
          ...
          // 如果有部分内容，认为是部分成功
          if (partialState.hasContent) {
            console.log(`[E2E] ⚠️ 部分响应完成: contentLength=${partialState.contentLength}, streaming=${partialState.isStreaming}`);
            return; // 不抛出错误，继续测试
          }

          throw error; // 没有任何内容，抛出原始错误
        });
